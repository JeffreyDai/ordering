...
import com.wzqj.ordering.common.mongo.model.MongoDBCursor;
import com.wzqj.ordering.common.mongo.model.MongoDBEntity;
import com.wzqj.ordering.common.mongo.model.MongoDBUpdate;
...
public class MongoDBUtil {
     ...
     * @param mongoDBCursor 查询实体
     */
    private static List<DBObject> find(final MongoDBCursor mongoDBCursor,
                                       DBObject dbObject, BasicDBObjectBuilder customField) {
        MongoDBCursorPreparer cursorPreparer = mongoDBCursor == null ? null : new MongoDBCursorPreparer() {
            public DBCursor prepare(DBCursor dbCursor) {
                ...
                if (mongoDBCursor.getLimit() <= 0 && mongoDBCursor.getSkip() <= 0 
                        && mongoDBCursor.getSortObject() == null) {
                    return dbCursor;
                }
     ...
     */
    private static List<DBObject> find(MongoDBCursor mongoDBCursor, DBObject query,
                                       MongoDBCursorPreparer cursorPreparer, BasicDBObjectBuilder customField) {
    ...
    private static DBObject getMapped(Map<String, Object> map) {
        ...
        while (iterable.hasNext()) {
            ...
            if (key.startsWith("$") && value instanceof Map) {
                ...
                Iterator<Map.Entry<String, Object>> iterator = conditionsMap.entrySet().iterator();
                while (iterator.hasNext()) {
                    Map.Entry<String, Object> entry1 = iterator.next();
                    String k = entry1.getKey();
                    Object conditionsValue = entry1.getValue();
